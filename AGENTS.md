...
- **`PLUGINS_WITH_UPDATE_DISABLED` is set on `:dirkwa` only.** A bundled plugin still appears in the App Store, and `bt-sensors-plugin-sk` carries upstream's own version string (`1.3.8-beta11`) because it is a branch of upstream, not a fork release. So any later published release reads as an available update — and accepting it installs into `~/.signalk/node_modules`, which is scanned FIRST and therefore **shadows the bundled copy from the next restart on**, silently reverting the BLE-API work. The env var only greys out the button; a deliberate install still wins, which is the intended escape hatch once a real release carries the change.
...
- **Bundling a PLUGIN from an unreleased branch into a variant** → two envs in `build-dirkwa.yml` plus two build args. `BT_SENSORS_REPO`/`BT_SENSORS_REF` name the fork branch; a `prepare` step packs it on the runner into `signalk-src/extra-packages/` (the plugin's `prepack` runs webpack, so the tarball ships built assets and needs no toolchain in the image). Then `bundle_plugins` (→ `BUNDLE_PLUGINS`) moves it into the nested tree and asserts it landed, and `plugins_with_update_disabled` (→ `PLUGINS_WITH_UPDATE_DISABLED`) greys out its App Store update button. Both build args default empty, so `:latest`/`:beta`/`:master` are untouched. Currently: `bt-sensors-plugin-sk` from `dirkwa/bt-sensors-plugin-sk:ble-gateway-api-support` (upstream PR #137 closed unmerged).
