...
- **The project-scoped `.npmrc` with `allow-scripts[]=@canboat/canboatjs` in the install stage.** npm >= 12 (pulled in by `npm install -g npm@latest`) skips dependency lifecycle scripts unless approved, and canboatjs's install script (`node-gyp rebuild || echo ...`) swallows the skip silently — that shipped images (2026-07) where canbus-canboatjs could not open can0 because `build/Release/canSocket.node` never got compiled. The `.npmrc` entry is the only accepted mechanism (`--allow-scripts` on the CLI errors with EALLOWSCRIPTS for project installs), and it is deliberately **targeted and project-scoped**: runtime appstore plugin installs happen in `/home/node/.signalk` and stay gated by npm 12 — a plugin needing a native build step at install time will not compile it there. That is a known, accepted consequence; do NOT "fix" it by setting `dangerously-allow-all-scripts` image-wide. A `test -f .../canSocket.node` assertion in the install stage fails the build loudly if the addon ever goes missing again.
- **canboat tools land in `/opt/canboat` with only three symlinks into `/usr/local/bin`, not a `PATH` entry — and only when `INSTALL_CANBOAT=1` (`:dirkwa`).** Deliberate on both counts. Symlinking exactly `analyzer`, `actisense-serial`, `candump2analyzer` means no other tool in the tarball can ever shadow a system binary. The `analyzer` name specifically is what signalk-server's `commandExists('analyzer')` probe checks to un-grey the native N2K connection options in the admin UI — renaming or relocating that symlink silently removes the option. The build-arg gate (default off) exists because the block sits in the shared base stage: unconditional install would change the connection editor in `:latest`/`:beta`/`:master`, which mirror upstream. `can-utils` is installed inside the same gated RUN, not the core apt list, for the same reason.
- **Bundled plugins are MOVED into `node_modules/signalk-server/node_modules/`, and the parent `node_modules/` copy is deleted.** Not redundant with the `@signalk` shuffle above — it is the same trap for a different reason. `extra-packages/` tarballs install into `/home/node/signalk/node_modules/`, which the server **never scans for plugins**: `getModulePaths()` returns exactly `configPath/node_modules` (`~/.signalk`) and `appPath/node_modules`, and `appPath` is the signalk-server *package* root, not the directory containing it. A plugin left in the parent is invisible — no error, no log line, it simply never loads. The `BUNDLE_PLUGINS` loop moves it and then **asserts** the destination exists even when the source was absent, so a pack step that silently staged nothing fails the build instead of shipping an image quietly missing the plugin.
- **`PLUGINS_WITH_UPDATE_DISABLED` exists but is currently set by no workflow.** It pairs with `BUNDLE_PLUGINS`: a bundled plugin still appears in the App Store, and one built from a branch carries upstream's own version string, so any later published release reads as an available update — and accepting it installs into `~/.signalk/node_modules`, which is scanned FIRST and therefore **shadows the bundled copy from the next restart on**, silently reverting whatever the branch added. The env var only greys out the button; a deliberate install still wins, which is the intended escape hatch once a real release carries the change. Set it alongside `BUNDLE_PLUGINS` whenever a variant bundles a plugin again.
...
- **Adding/removing a PR from the `:dirkwa` stack** → edit the `PRS:` env in `.github/workflows/build-dirkwa.yml`. That's the only source of truth for PRs (the stack also carries `BRANCHES:` for changes with no upstream PR and `N2K_SIGNALK_*` for an unreleased server dependency — all three feed the same composite key). Composite state key auto-updates. PRs that get **merged upstream** are skipped automatically with a workflow warning (a merged PR's branch add/add-conflicts with the master that now contains it); the skip drops the PR's entry from the composite key, so the transition itself triggers the rebuild that picks the change up via master. Still remove merged PRs from `PRS:` when convenient — the auto-skip costs one API warning per run and hides them from the `io.dirkwa.signalk.prs` label either way.
...
- **Bundling a PLUGIN from an unreleased branch into a variant** → a pair of `<PLUGIN>_REPO`/`<PLUGIN>_REF` envs in the variant's workflow plus two build args. The envs name the fork branch and feed the composite key (so pushing to the branch triggers a rebuild); a `prepare` step packs the branch on the runner into `signalk-src/extra-packages/` (pack there rather than in the image, so a plugin whose `prepack` builds assets needs no toolchain in the image). Then `bundle_plugins` (→ `BUNDLE_PLUGINS`) moves it into the nested tree and asserts it landed, and `plugins_with_update_disabled` (→ `PLUGINS_WITH_UPDATE_DISABLED`) greys out its App Store update button — set both together, see the note above for why. Both build args default empty, so a variant that sets neither is untouched. **Currently no variant bundles a plugin.** `:dirkwa` carried `bt-sensors-plugin-sk` from `dirkwa/bt-sensors-plugin-sk:ble-gateway-api-support` (upstream PR #137 closed unmerged) until it was removed on 2026-09-17; `git log -- .github/workflows/build-dirkwa.yml` has the full wiring if it is ever needed as a worked example.
